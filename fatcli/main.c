...
// FAT CLI
//  Command line interface for FAT images
//  Uses FatFs http://elm-chan.org/fsw/ff/00index_e.html
//
...
{
    fputs("Usage: fatcli <image> <command> [<cmd args>]\n", stderr);
    fputs("Commands:\n", stderr);
    fputs(" ls [<dir>]            Lists files in root / given directory\n", stderr);
    fputs(" mkdir <dir>           Creates the given directory\n", stderr);
    fputs(" mkfs <sectors>        Make a new filesystem (using 512 byte sectors)\n", stderr);
    fputs(" read <ipath> <epath>  Reads a file from the image\n", stderr);
    fputs(" rm <name>             Removes the given file\n", stderr);
    fputs(" write <ipath> <epath> Writes a file to the image\n", stderr);
    fputs("\n", stderr);
    fputs("ipath refers to the IMAGE path, epath refers to the EXTERNAL path\n", stderr);
    return 1;
}
...
int main(int argc, char ** argv)
{
    ...
    // Open image file
    diskImage = fopen(argv[1], openMode);
    if (diskImage == NULL)
    {
...
DRESULT disk_read(BYTE pdrv, BYTE * buff, DWORD sector, BYTE count)
{
    ...
    if (fseek(diskImage, sector * SECTOR_SIZE, SEEK_SET) != 0)
    {
        perror("(Read) File seek failed");
        return RES_ERROR;
    }

    // Read data
    if (fread(buff, 1, bytes, diskImage) != bytes)
    {
        // Eof?
        if (feof(diskImage))
            fputs("(Read) EOF encounted during read\n", stderr);
        else
...
DRESULT disk_write(BYTE pdrv, const BYTE * buff, DWORD sector, BYTE count)
{
    ...
    if (fseek(diskImage, sector * SECTOR_SIZE, SEEK_SET) != 0)
    {
        perror("(Write) File seek failed");
        return RES_ERROR;
    }

    // Write data
    if (fwrite(buff, 1, bytes, diskImage) != bytes)
    {
...
DRESULT disk_ioctl(BYTE pdrv, BYTE cmd, void* buff)
{
    ...
    switch (cmd)
    {
        ...
        case GET_SECTOR_COUNT:
            // Get sector count (mkfs only)
            *((DWORD *) buff) = sectorCount;
            break;

        case GET_BLOCK_SIZE:
            // Get erase block size (flash devices only)
            *((DWORD *) buff) = 1;
            break;

        case CTRL_ERASE_SECTOR:
            // Erase flash sector (flash devices only)
            break;
...
DWORD get_fattime(void)
{
    ...
    struct tm * tmStruct = gmtime(&timeVal);

    // Generate DWORD value
    return  (tmStruct->tm_year - 80) << 25 |
            (tmStruct->tm_mon + 1)   << 21 |
            (tmStruct->tm_mday)      << 16 |
            (tmStruct->tm_hour)      << 11 |
            (tmStruct->tm_min)       << 5 |
            (tmStruct->tm_sec / 2);
...
// Convert Latin-1 char to unicode
